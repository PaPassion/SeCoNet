...
def whole_process(m, repeat):
    ...
    # sir dynamics
    #s, i, r
    susceptible[0], infected[0], recovered[0] = transmission.general_sir(nodes)
    (female_susceptible[0], female_infected[0], female_recovered[0],
     male_susceptible[0], male_infected[0], male_recovered[0]) = transmission.gender_sir(nodes)
    ...
    for day in range(1, total_ticks):
        ...
        susceptible[day], infected[day], recovered[day] = transmission.general_sir(nodes)
        (female_susceptible[day], female_infected[day], female_recovered[day],
         male_susceptible[day], male_infected[day], male_recovered[day]) = transmission.gender_sir(nodes)

        #print(day, susceptible[day], infected[day], recovered[day])
    ...
    fig_sir = plots.general_sir(susceptible, infected, recovered)
    fig_sir.show()

    fig_gender = plots.gender_sir(female_susceptible, female_infected, female_recovered, male_susceptible,male_infected, male_recovered)
    fig_gender.show()
    ...
    files.record_sir(path, susceptible, infected, recovered, female_susceptible, female_infected, female_recovered,
                     male_susceptible, male_infected, male_recovered)
    files.record_incidence(path, incidences, cumulative, female_incidences, female_cumulative, male_incidences, male_cumulative)
