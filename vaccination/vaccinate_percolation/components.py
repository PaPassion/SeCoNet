...
class Node:
    ...
    def fitness_distribution(self):
        while self.fitness <= 0:
            self.fitness = np.random.poisson(300)
...
def percolation(nodes, links):
    ...
    results = nx.percolation_centrality(graph, 'percolation', states)

    for node in nodes:
        node.centrality = results[node]

    #print([node.centrality for node in nodes])
