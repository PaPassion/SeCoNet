...
def vaccinate_session(nodes, total_vaccination, actual_vaccinated):
    objects = [node for node in nodes if node.newbie == 1]
    objects.sort(key = lambda x: len(x.neighbors), reverse=True)
    count = 0

    for node in objects:
        if count < total_vaccination // 4:
            for neighbor in node.neighbors:
                if neighbor.status == 0 and node.age < 26:
                    neighbor.status = -2
                    count += 1
                if count >= total_vaccination // 4:
                    print(count, total_vaccination // 4)
                    break
        if count >= total_vaccination // 4:
            print(count, total_vaccination // 4)
            break
    ...
    print(count, total_vaccination // 4, actual_vaccinated)