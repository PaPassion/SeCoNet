...
def vaccinate_session(nodes, total_vaccination, actual_vaccinated):
    options = [node for node in nodes if node.age < 26 and node.newbie == 1 and node.status == 0]
    eligible_counts = len(options)
    vaccinate_number = min([eligible_counts, total_vaccination // 4])

    candidates = random.sample(options, vaccinate_number)
