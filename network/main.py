...
def initial_network(m_0, nodes,all_links, active_links, average_age_difference, number_of_nodes, number_of_links,
    ...
    print(len(options), heterogeneity[-1], clustering[-1], avgpath[-1])
...
def whole_process(m, repeat):
    ...
    # fitness_data = []
    # for node in nodes:
    #     fitness_data.append(node.fitness)
    # print(np.mean(fitness_data), np.std(fitness_data))
    ...
    for day in range(1, total_ticks):
        ...
        print(day, len(options), heterogeneity[-1], clustering[-1], avgpath[-1])

        # print(len(number_of_nodes), len(number_of_links), len(assortativity), len(avg),
        #       len(primary), len(secondary), len(removed))
    ...
    fig2 = plots.fitting(m, epsilon, nodes)
    fig2.show()


    files.record_network(path, number_of_nodes, number_of_links, assortativity, primary, secondary, removed, avg,
                         heterogeneity, clustering, avgpath)
